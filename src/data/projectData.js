...
const projectBase = [
  {
    ...
    detailDescription: {
      ...
      yue: "本專案為多個公部門單位建置 7 套以上智慧聊天機械人系統。採用 GraphRAG 架構，知識庫以知識圖譜形式儲存喺 Neo4j，結合 Text-to-SQL 畀用戶以自然語言查詢結構化資料。系統部署喺 GCP 同地端混合架構，透過 AI Agent 自動化行政流程，大幅提升公務效率。",
    },
    highlights: {
      ...
      yue: ["建置 7+ 套聊天機械人，涵蓋法規查詢、案件追蹤、內部知識庫", "GraphRAG + Neo4j 知識圖譜做跨文件關聯推理", "Text-to-SQL 畀非技術人員直接用自然語言查資料庫", "GCP + 地端混合部署，符合政府資安規範", "AI Agent 自動化公文分類、摘要同派案流程"],
    },
    metrics: {
  ...
  },
  {
    ...
    description: {
      ...
      yue: "結合 LLM Gateway 同 Playwright 瀏覽器自動化嘅財務分析平台。多 Agent 協作（新聞 / 財務 / 交易 / 數據），配合 Safety Guard 安全層同完整 Artifact 追溯。",
    },
    detailDescription: {
      ...
      yue: "VICI 係一套生產級 LLM Gateway + 多代理財務分析系統。Gateway 層採用 Provider 抽象設計，支援 OpenAI / Claude / Mock 一鍵切換，內建 Safety Guard 五層安全架構。四個專業 Agent 各司其職，透過 Playwright 無頭瀏覽器自動抓取股票行情同新聞，再由 LLM 生成帶情緒分析嘅投資報告同簡報。每次運行產出完整 Artifact，支援 Deterministic Dry-run 離線測試。",
    },
    highlights: {
      zh: [
    ...
    metrics: {
      ...
      yue: [{ label: "Agent 數", value: "4" }, { label: "安全層級", value: "5 層" }, { label: "LLM 提供商", value: "3+" }],
    },
  },
  {
    ...
    detailDescription: {
      ...
      yue: "本專案針對醫療影像領域，用 PyTorch 同 ResNet 預訓練模型，建立自動化胸部 X 光片異常檢測系統。透過遷移學習同資料增強，喺有限標註資料下仲達到 AUC 0.95。系統採用雲地混合部署，可同時服務醫院內部同遠端診斷，為醫師提供即時輔助診斷建議。",
    },
    highlights: {
      ...
      yue: ["AUC 達 0.95，準確率 94%，具臨床輔助價值", "遷移學習 + 資料增強克服標註資料不足問題", "雲地混合部署，支援院內同遠端診斷", "自動生成熱力圖標示異常區域，提升醫師判讀效率", "符合醫療資料隱私規範嘅安全架構"],
    },
    metrics: {
  ...
  },
  {
    ...
    detailDescription: {
      ...
      yue: "本專案整合 15+ 種資料來源，包含氣象局即時氣象、船舶靜態資料庫、同全台灣周圍海域 AIS 即時串流資料。所有資料匯入 ClickHouse 即時流式庫，經特徵工程後餵入深度學習模型，對船舶擱淺風險同異常航行行為做 7×24 不間斷預測，協助海巡單位提前預警。",
    },
    highlights: {
      ...
      yue: ["整合 15+ 種異質資料來源（氣象、AIS、船舶靜態資料）", "ClickHouse 即時流式庫處理 20TB+ 數據", "深度學習模型 7×24 不間斷預測擱淺同異常行為", "地端部署符合國安等級資料處理規範", "為海巡單位建立預警系統，提前 30 分鐘發出警報"],
    },
    metrics: {
  ...
  },
  {
    ...
    description: {
      ...
      yue: "比較 SARIMA、Holt-Winters、Prophet 三種模型喺 COVID-19 干擾下嘅預測表現，設計三組實驗量化外部衝擊對時間序列預測嘅影響。",
    },
    detailDescription: {
      ...
      yue: "本專案探討 COVID-19 疫情點樣影響時間序列預測模型嘅準確度。透過嚴謹實驗設計，將數據分為三組，分別以 SARIMA、Holt-Winters、Prophet 進行建模與預測，比較各模型喺唔同情境下嘅表現，提出面對外部干擾時嘅最佳預測策略。",
    },
    highlights: {
      zh: [
    ...
    metrics: {
      ...
      yue: [{ label: "預測模型", value: "3" }, { label: "實驗組數", value: "3" }, { label: "機構", value: "Uni Mannheim" }],
    },
  },
  {
    icon: <FaDice size={28} />,
    categoryIcon: <FaCode size={16} />,
    category: { zh: "演算法", en: "Algorithm", yue: "演算法" },
    ...
    description: {
      ...
      yue: "3×3 老虎機遊戲引擎，結合 Monte Carlo 模擬同啟發式搜索算法自動優化轉軸配置，喺目標 RTP 95% 同勝率 ≥ 55% 約束下搵出最佳符號分布。全棧實現含 FastAPI 後端同 React 前端。",
    },
    detailDescription: {
      ...
      yue: "係一套老虎機遊戲引擎同機率優化系統。核心挑戰在於點樣設計轉軸配置令遊戲同時滿足 RTP 同勝率雙目標。系統以啟發式搜索迭代 800 步，每步透過 Monte Carlo 模擬 50,000 局評估配置適應度，最終以 100,000 局精確驗證。5 種符號搭配 5 種獲勝模式，透過符號分布變異同適應性搜索策略，喺巨大搜索空間中高效收斂。全棧實現支援 CLI 同 API 雙模式，React 前端可即時體驗遊戲同查看統計。",
    },
    highlights: {
      zh: [
      ...
      yue: [
        "啟發式搜索算法：800 步迭代優化，平衡探索與利用，自動收斂至目標 RTP/勝率",
        "Monte Carlo 模擬引擎：每次評估 50,000 局、精確驗證 100,000 局，確保統計信賴度",
        "多目標優化：同時滿足 RTP ≥ 0.95 同 Win Rate ≥ 0.55 嘅約束條件",
        "5 種獲勝模式 × 5 種符號賠率，支援 RNG seed 確保結果可重現",
        "全棧實現：FastAPI + React + Vite，CLI / API 雙模式，Makefile 自動化部署",
      ],
    },
    metrics: {
      ...
      yue: [{ label: "目標 RTP", value: "95%" }, { label: "模擬次數", value: "100K" }, { label: "獲勝模式", value: "5 種" }],
    },
  },
  {
    ...
    title: {
      ...
      yue: "AutoLLM — No-Code RAG 聊天機械人平台",
    },
    description: {
      ...
      yue: "生產級 RAG 平台，支援文檔上傳、向量檢索、多 LLM 切換（OpenAI / Gemini / Claude），配合 Redis + PostgreSQL 雙層存儲同 Docker 微服務部署。",
    },
    detailDescription: {
      ...
      yue: "AutoLLM 係一套 No-Code RAG 聊天機械人平台。用戶上傳文件後，系統自動完成解析、分塊、Embedding 生成同向量儲存（pgvector），再透過餘弦相似度檢索相關文檔塊，構建上下文送入 LLM 生成帶引用嘅答案。架構採用 FastAPI + Next.js 全棧，Redis 做短期對話緩存、PostgreSQL 做永久審計，Provider 抽象層可一鍵切換 OpenAI / Gemini / Claude。全服務以 Docker Compose 編排，支援流式回答、JWT 認證同速率限制。",
    },
    highlights: {
      zh: [
      ...
      yue: [
        "完整 RAG 流程：上傳 → 分塊 → Embedding → 向量檢索 → LLM 生成帶引用答案",
        "Provider 抽象層（策略模式），一鍵切換 OpenAI / Gemini / Claude",
        "Redis 短期緩存 + PostgreSQL 永久審計嘅雙層對話存儲架構",
        "pgvector 餘弦距離檢索，可配置 chunk_size / overlap / top_k",
        "Docker Compose 微服務部署，SSE 流式回答 + JWT 認證 + 速率限制",
      ],
    },
    metrics: {
      ...
      yue: [{ label: "LLM 提供商", value: "3+" }, { label: "API 端點", value: "15+" }, { label: "部署", value: "Docker" }],
    },
  },
];
