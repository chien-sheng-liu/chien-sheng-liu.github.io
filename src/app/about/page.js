...
const aggregateSpeechStats = (categories) => {
  ...
  return {
    talks,                 // 總場次
    audience,              // 總參與人數（數字）
    audiencePlus: anyPlus, // 是否加上 '+'
    categories: categories.length, // 分類數
  };
};
  ...
  return (
    <div className="relative min-h-screen text-[var(--color-white)] overflow-hidden">
      ...
      <div className="absolute inset-0">
        ...
        {/* Floating AI/Data themed elements */}
        <motion.div
          className="absolute top-1/4 right-1/5 w-8 h-8 border-2 border-[var(--color-electric-blue)]/20 rounded-full"
          variants={floatingVariants}
          animate="animate"
        />
        <motion.div
          className="absolute top-2/3 left-1/6 w-6 h-6 bg-[var(--color-violet-glow)]/30 rounded-lg"
          variants={floatingVariants}
          animate="animate"
      ...
      <div className="relative z-10 container mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        >
          ...
          >
            {pageText.subtitle.split('數據領袖').map((part, index) =>
              index === 0 ? (
                <span key={index}>{part}<span className="text-[var(--color-electric-blue)] font-semibold">數據領袖</span></span>
              ) : (
                part.split('產品建構者').map((subpart, subindex) =>
                  subindex === 0 ? (
                    <span key={`${index}-${subindex}`}> 與 <span className="text-[var(--color-violet-glow)] font-semibold">產品建構者</span>{subpart}</span>
                  ) : (
          ...
          {/* Top Stats Section（保留你原本的 stats 陣列） */}
          <motion.div
          ...
          {highlights.map((highlight, index) => (
            ...
            >
              ...
              <div className="relative text-center">
                <motion.div
                  className={`inline-flex items-center justify-center w-16 h-16 bg-gradient-to-br ${highlight.color} rounded-2xl mb-6 shadow-lg`}
                ...
                <h3 className="text-2xl font-bold text-white mb-4 group-hover:text-[var(--color-electric-blue)] transition-colors duration-300">
                  {highlight.title}
                </h3>
        ...
        <div className="flex flex-col items-center w-full space-y-24">
          ...
          <section className="w-full">
            <motion.h2
              className="text-4xl font-bold mb-16 text-center bg-clip-text text-transparent bg-gradient-to-r from-white to-[var(--color-electric-blue)]"
              initial={{ opacity: 0, y: 20 }}
          ...
          <section className="w-full">
            <motion.h2
              className="text-4xl font-bold mb-16 text-center bg-clip-text text-transparent bg-gradient-to-r from-white to-[var(--color-violet-glow)]"
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              transition={{ duration: 0.8 }}
          ...
          <section className="w-full">
            ...
            >
              <div className="text-center p-6 bg-gradient-to-br from-[var(--color-gray-800)]/60 to-[var(--color-gray-800)]/30 rounded-2xl border border-[var(--color-gray-700)]/50">
                <div className="text-3xl font-bold text-[var(--color-electric-blue)] mb-2">
                  {statsAuto.talks}
                </div>
                <div className="text-[var(--color-gray-300)]">場演講</div>
              </div>
              <div className="text-center p-6 bg-gradient-to-br from-[var(--color-gray-800)]/60 to-[var(--color-gray-800)]/30 rounded-2xl border border-[var(--color-gray-700)]/50">
                <div className="text-3xl font-bold text-[var(--color-violet-glow)] mb-2">
                  {statsAuto.audience}{statsAuto.audiencePlus ? '+' : ''}
                </div>
                <div className="text-[var(--color-gray-300)]">位參與者</div>
              </div>
              <div className="text-center p-6 bg-gradient-to-br from-[var(--color-gray-800)]/60 to-[var(--color-gray-800)]/30 rounded-2xl border border-[var(--color-gray-700)]/50">
                <div className="text-3xl font-bold text-[var(--color-electric-blue)] mb-2">
                  {statsAuto.categories}
                </div>
                <div className="text-[var(--color-gray-300)]">個主要領域</div>
              </div>
            </motion.div>
            ...
            >
              {speechCategories.map((category, index) => (
                <motion.div
                  key={index}
                  variants={itemVariants}
                  className="group relative overflow-hidden bg-gradient-to-br from-[var(--color-gray-800)]/90 to-[var(--color-gray-800)]/60 backdrop-blur-xl rounded-3xl p-6 border border-[var(--color-gray-700)]/50 transition-all duration-500 hover:border-[var(--color-electric-blue)]/50 hover:shadow-2xl hover:shadow-[var(--color-electric-blue)]/10"
                  ...
                  <div className="relative">
                    ...
                    <div className="flex items-center justify-between mb-6">
                      <div className="flex items-center space-x-3">
                        <div className={`p-3 bg-gradient-to-br ${category.color} rounded-xl text-white shadow-lg`}>
                          {category.icon}
                        </div>
                        <div>
                          ...
                          {/* 這裡也用自動統計（每個分類自己的總數） */}
                          {(() => {
                            const { sum, hasPlus } = sumAudience(category.speeches);
                            return (
                              <p className="text-sm text-[var(--color-gray-400)]">
                                {category.speeches.length} 場演講 • {sum}{hasPlus ? '+' : ''} 參與者
                              </p>
                            );
                          })()}
                        </div>
            ...
            >
              {languages.map((lang, index) => (
                <motion.div
                  key={index}
                  variants={itemVariants}
                  className="group relative overflow-hidden bg-gradient-to-br from-[var(--color-gray-800)]/80 to-[var(--color-gray-800)]/40 backdrop-blur-xl rounded-3xl p-8 border border-[var(--color-gray-700)]/50 transition-all duration-500 hover:border-[var(--color-electric-blue)]/50 hover:shadow-2xl hover:shadow-[var(--color-electric-blue)]/10 text-center"
                ...
                >
                  ...
                  <div className="relative">
                    <motion.div
                      className={`inline-flex items-center justify-center w-16 h-16 bg-gradient-to-br ${lang.color} rounded-2xl mb-6 shadow-lg`}
                      whileHover={{ rotate: 5, scale: 1.1 }}
                      transition={{ type: "spring", stiffness: 300 }}
                    >
                    ...
                    <p className="text-[var(--color-gray-400)] mb-3 group-hover:text-[var(--color-gray-300)] transition-colors duration-300">
                      {lang.description}
                    </p>

                    <span className="px-4 py-2 bg-[var(--color-electric-blue)]/10 border border-[var(--color-electric-blue)]/20 rounded-full text-sm font-medium text-[var(--color-electric-blue)]">
                      {lang.level}
                    </span>
                  </div>
                </motion.div>
            ...
            >
              {skills.map((skillCategory, index) => (
                <motion.div
                  key={index}
                  variants={itemVariants}
                  className="group relative overflow-hidden bg-gradient-to-br from-[var(--color-gray-800)]/80 to-[var(--color-gray-800)]/40 backdrop-blur-xl rounded-2xl p-6 border border-[var(--color-gray-700)]/50 transition-all duration-500 hover:border-[var(--color-electric-blue)]/50 hover:shadow-xl hover:shadow-[var(--color-electric-blue)]/10 text-center"
                  whileHover={{ y: -5, scale: 1.02 }}
                >
                  ...
                  <div className="relative">
                    <motion.div
                      className={`inline-flex items-center justify-center w-12 h-12 bg-gradient-to-br ${skillCategory.color} rounded-xl mb-4 shadow-lg`}
                      whileHover={{ rotate: 5, scale: 1.1 }}
                      transition={{ type: "spring", stiffness: 300 }}
                    >
                    ...
                    <h3 className="text-lg font-bold text-white mb-4 group-hover:text-[var(--color-electric-blue)] transition-colors duration-300">
                    ...
                    <div className="flex flex-wrap gap-2 justify-center">
                      {skillCategory.items.map((skill, i) => (
                        <span
                          key={i}
                          className="inline-flex items-center px-3 py-2 bg-[var(--color-gray-900)]/40 rounded-full border border-[var(--color-gray-600)]/30 hover:border-[var(--color-electric-blue)]/50 transition-all duration-300 text-sm font-medium text-white hover:text-[var(--color-electric-blue)] cursor-default"
                        >
                          <span className="text-[var(--color-electric-blue)] mr-2 text-xs">
                            {skill.icon}
                          </span>
                          {skill.name}
                        </span>
