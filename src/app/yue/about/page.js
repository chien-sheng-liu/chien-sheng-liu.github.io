...
import { highlights, experiences, education, speechCategories, languages, skills, stats, pageText } from './aboutData';
...
const floatingVariants = { animate: { y: [-8, 8, -8], x: [-5, 5, -5], rotate: [-2, 2, -2], transition: { duration: 6, repeat: Infinity, ease: "easeInOut" } } };

const sumAudience = (speeches) => {
  let sum = 0, hasPlus = false;
  speeches.forEach(s => { const raw = String(s?.audience ?? ''); const n = parseInt(raw.replace(/[^\d]/g, ''), 10); if (!Number.isNaN(n)) sum += n; if (raw.includes('+')) hasPlus = true; });
  return { sum, hasPlus };
};
const aggregateSpeechStats = (categories) => {
  let talks = 0, audience = 0, anyPlus = false;
  categories.forEach(cat => { const list = cat?.speeches ?? []; talks += list.length; const { sum, hasPlus } = sumAudience(list); audience += sum; if (hasPlus) anyPlus = true; });
...
const ExpandableSpeechList = ({ speeches, initiallyVisible = 2 }) => {
  ...
  return (
    <motion.div layout className="space-y-3">
      {toShow.map((speech, i) => (
        <motion.div key={`${speech.title}-${i}`} layout className="p-3 bg-[var(--color-gray-900)]/30 rounded-lg border border-[var(--color-gray-600)]/20 group/item hover:border-[var(--color-electric-blue)]/30 transition-colors duration-300">
          <div className="flex items-start justify-between mb-2">
            <h4 className="text-sm font-semibold text-white group-hover/item:text-[var(--color-electric-blue)] transition-colors duration-300 leading-tight">{speech.title}</h4>
            <span className="text-xs text-[var(--color-gray-500)] whitespace-nowrap ml-2">{speech.date}</span>
          </div>
          <div className="flex items-center justify-between text-xs text-[var(--color-gray-400)]">
            <span>{speech.company}</span>
            <div className="flex items-center space-x-1"><FaMicrophone className="w-3 h-3" /><span>{speech.audience}</span></div>
          </div>
        </motion.div>
      ))}
      {speeches.length > initiallyVisible && (
        <div className="text-center pt-2">
          <button onClick={() => setExpanded(v => !v)} className="inline-flex items-center gap-1 text-xs text-[var(--color-electric-blue)] hover:underline">
            {expanded ? "收合演講" : `+${speeches.length - initiallyVisible} 更多演講`}
  ...
  return (
    <div className="relative min-h-screen text-[var(--color-white)] overflow-hidden">
      <div className="absolute inset-0">
        ...
        <motion.div className="absolute top-1/3 left-1/2 text-[var(--color-electric-blue)]/20 text-2xl" variants={floatingVariants} animate="animate" style={{animationDelay: '3s'}}><FaRobot /></motion.div>
      </div>
      ...
      <div className="relative z-10 container mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <motion.div className="text-center mb-16" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
          <motion.h1 className="text-5xl md:text-6xl font-bold mb-6 bg-clip-text text-transparent bg-gradient-to-r from-white via-[var(--color-electric-blue)] to-[var(--color-violet-glow)]" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.2 }}>{pageText.title}</motion.h1>
          <motion.p className="text-xl text-[var(--color-gray-300)] max-w-4xl mx-auto leading-relaxed mb-12" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.4 }}>{pageText.subtitle}</motion.p>
          <motion.div className="grid grid-cols-2 md:grid-cols-4 gap-6 max-w-4xl mx-auto mb-16" variants={containerVariants} initial="hidden" animate="visible">
            {stats.map((stat, index) => (
              <motion.div key={index} variants={itemVariants} className="relative p-6 bg-gradient-to-br from-[var(--color-gray-800)]/60 to-[var(--color-gray-800)]/30 backdrop-blur-lg rounded-2xl border border-[var(--color-gray-700)]/50 text-center group hover:border-[var(--color-electric-blue)]/50 transition-all duration-300" whileHover={{ y: -5, scale: 1.05 }}>
                ...
                <div className="text-3xl font-bold text-white mb-1">{stat.value}<span className="text-[var(--color-electric-blue)]">{stat.suffix}</span></div>
              ...
              <div className="relative text-center">
                ...
                <div className="flex flex-wrap justify-center gap-2">
                  {highlight.metrics.map((metric, i) => (
                    <span key={i} className="px-3 py-1 bg-[var(--color-electric-blue)]/10 border border-[var(--color-electric-blue)]/20 rounded-full text-xs font-medium text-[var(--color-electric-blue)]">{metric}</span>
                  ))}
                </div>
              </div>
        ...
        <div className="flex flex-col items-center w-full space-y-24">
          <section className="w-full">
            <motion.h2 className="text-4xl font-bold mb-16 text-center bg-clip-text text-transparent bg-gradient-to-r from-white to-[var(--color-electric-blue)]" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }}>{pageText.sections.experience}</motion.h2>
            <CreativeTimeline events={experiences} />
          </section>

          <section className="w-full">
            <motion.h2 className="text-4xl font-bold mb-16 text-center bg-clip-text text-transparent bg-gradient-to-r from-white to-[var(--color-violet-glow)]" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }}>{pageText.sections.education}</motion.h2>
            <CreativeTimeline events={education} />
          </section>

          <section className="w-full">
            <motion.h2 className="text-4xl font-bold mb-16 text-center bg-clip-text text-transparent bg-gradient-to-r from-white to-[var(--color-electric-blue)]" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }}>{pageText.sections.speeches}</motion.h2>
            <motion.div className="grid grid-cols-2 md:grid-cols-4 gap-6 max-w-4xl mx-auto mb-10" variants={containerVariants} initial="hidden" whileInView="visible" viewport={{ once: true }}>
              {[
                { label: '總場次', value: String(statsAuto.talks) },
                { label: '分類', value: String(statsAuto.categories) },
                { label: '參與人數', value: `${statsAuto.audience}${statsAuto.audiencePlus ? '+' : ''}` },
                { label: 'Highlights', value: '精選' },
              ].map((s, i) => (
              ...
              {speechCategories.map((category, index) => (
                <motion.div key={index} variants={itemVariants} className="group relative overflow-hidden bg-gradient-to-br from-[var(--color-gray-800)]/80 to-[var(--color-gray-800)]/40 backdrop-blur-xl rounded-3xl p-8 border border-[var(--color-gray-700)]/50 transition-all duration-500 hover:border-[var(--color-electric-blue)]/50 hover:shadow-2xl hover:shadow-[var(--color-electric-blue)]/10" whileHover={{ y: -8, scale: 1.02 }}>
                  ...
                  <div className="relative text-center">
                    ...
                    <p className="text-[var(--color-gray-400)] mb-2 group-hover:text-[var(--color-gray-300)] transition-colors duration-300">場次：<span className="font-semibold text-[var(--color-electric-blue)]">{category.speeches.length}</span> · 人數：<span className="font-semibold text-[var(--color-electric-blue)]">{category.totalAudience}</span></p>
          ...
          <section className="w-full">
            <motion.h2 className="text-4xl font-bold mb-16 text-center bg-clip-text text-transparent bg-gradient-to-r from-white to-[var(--color-electric-blue)]" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }}>{pageText.sections.languages}</motion.h2>
            <motion.div className="grid grid-cols-1 md:grid-cols-3 gap-8" variants={containerVariants} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.2 }}>
          ...
          <section className="w-full">
            <motion.h2 className="text-4xl font-bold mb-16 text-center bg-clip-text text-transparent bg-gradient-to-r from-white to-[var(--color-violet-glow)]" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }}>專業技能</motion.h2>
        ...
        <motion.div className="text-center mt-24" initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }}>
          <div className="max-w-4xl mx-auto">
            ...
            <div className="inline-flex items-center space-x-2 px-8 py-3 bg-[var(--color-electric-blue)]/10 border border-[var(--color-electric-blue)]/30 rounded-full text-[var(--color-electric-blue)]"><FaLightbulb className="animate-pulse" /><span className="font-medium">{pageText.cta.tagline}</span></div>
          </div>
        </motion.div>
      </div>
    </div>
