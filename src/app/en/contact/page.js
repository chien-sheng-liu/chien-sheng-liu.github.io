      ...
      <div className="relative z-10 container mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <motion.div className="text-center mb-16" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
          ...
          <motion.p className="text-xl text-[var(--color-gray-300)] max-w-3xl mx-auto leading-relaxed mb-8" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.4 }}>
            I’d love to discuss potential collaborations, project ideas, or anything related to <span className="text-[var(--color-electric-blue)] font-semibold">Data Science</span> and <span className="text-[var(--color-violet-glow)] font-semibold">AI</span>.
          </motion.p>
          ...
          <motion.div className="relative" initial={{ opacity: 0, x: 50 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, amount: 0.3 }} transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }}>
            <div className="relative overflow-hidden bg-gradient-to-br from-[var(--color-gray-800)]/90 to-[var(--color-gray-800)]/60 backdrop-blur-xl rounded-3xl p-8 border border-[var(--color-gray-700)]/50 shadow-2xl">
              <div className="text-center mb-8">
                ...
                <p className="text-[var(--color-gray-400)] text-sm">Fill the form to open your email app with content prefilled</p>
              </div>

              <form className="space-y-6" onSubmit={onSubmit}>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <label htmlFor="name" className="block text-sm font-semibold text-white/90">Name <span className="text-red-400">*</span></label>
                    <motion.input type="text" id="name" placeholder="Your name" value={name} onChange={(e) => setName(e.target.value)} className="w-full bg-[var(--color-gray-900)]/50 border border-[var(--color-gray-600)]/30 rounded-xl py-4 px-4 focus:outline-none focus:ring-2 focus:ring-[var(--color-electric-blue)] focus:border-transparent transition-all duration-300 text-white placeholder-gray-500" whileFocus={{ scale: 1.02 }} />
                  </div>
                  <div className="space-y-2">
                    <label htmlFor="email" className="block text-sm font-semibold text-white/90">Email <span className="text-red-400">*</span></label>
                    <motion.input type="email" id="email" placeholder="Your email (sender)" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full bg-[var(--color-gray-900)]/50 border border-[var(--color-gray-600)]/30 rounded-xl py-4 px-4 focus:outline-none focus:ring-2 focus:ring-[var(--color-electric-blue)] focus:border-transparent transition-all duration-300 text-white placeholder-gray-500" whileFocus={{ scale: 1.02 }} />
                  </div>
                </div>
                <div className="space-y-2">
                  <label htmlFor="subject" className="block text-sm font-semibold text-white/90">Subject</label>
                ...
                <div className="space-y-2">
                  <label htmlFor="message" className="block text-sm font-semibold text-white/90">Message</label>
                  <motion.textarea id="message" placeholder="Write your message here" rows={6} value={message} onChange={(e) => setMessage(e.target.value)} className="w-full bg-[var(--color-gray-900)]/50 border border-[var(--color-gray-600)]/30 rounded-xl py-4 px-4 focus:outline-none focus:ring-2 focus:ring-[var(--color-electric-blue)] focus:border-transparent transition-all duration-300 text-white placeholder-gray-500" whileFocus={{ scale: 1.01 }} />
                </div>
                ...
                <div className="text-center text-xs text-[var(--color-gray-400)] mt-2 space-x-3">
                  <button type="button" onClick={openWithGmail} className="underline hover:text-white">Use Gmail (force)</button>
                  <span>·</span>
                  <button type="button" onClick={openWithOutlookWeb} className="underline hover:text-white">Use Outlook Web</button>
                </div>
                {feedback && (
                  <div aria-live="assertive" className="text-sm mt-2 p-3 rounded-lg border border-red-500/40 bg-red-500/10 text-red-200">{feedback}</div>
                )}
                <div className="mt-6 pt-6 border-t border-[var(--color-gray-600)]/30">
                  <p className="text-center text-sm text-[var(--color-gray-500)]">Submitting opens your email app; you still need to review and press send.</p>
                </div>
              </form>
        ...
        <motion.div className="text-center mt-24" initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }}>
          <div className="max-w-3xl mx-auto">
            ...
            <p className="text-[var(--color-gray-400)] mb-8 leading-relaxed">Whether it’s AI development, career consulting, or any data challenge, I’m happy to discuss and provide professional advice.</p>
