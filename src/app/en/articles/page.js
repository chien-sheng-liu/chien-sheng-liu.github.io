    ...
    <div className="relative min-h-screen text-[var(--color-white)] overflow-hidden">
      ...
      <div className="relative z-10 container mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="text-center mb-14">
          <h1 className="text-5xl md:text-6xl font-bold mb-4 bg-clip-text text-transparent bg-gradient-to-r from-white via-[var(--color-electric-blue)] to-[var(--color-violet-glow)]">Articles</h1>
          <p className="text-[var(--color-gray-300)] text-lg max-w-3xl mx-auto">Writing on AI, data engineering, and taking products to production.</p>
        </div>
          ...
          {posts.map((post) => (
            <article key={post.slug} className="group relative overflow-hidden bg-gradient-to-br from-[var(--color-gray-800)]/80 to-[var(--color-gray-800)]/40 backdrop-blur-xl rounded-2xl p-6 border border-[var(--color-gray-700)]/50 transition-all duration-300 hover:border-[var(--color-electric-blue)]/50 hover:shadow-2xl hover:shadow-[var(--color-electric-blue)]/10">
              ...
              <div className="relative">
                ...
                <Link href={`/en/articles/${post.slug}`} className="inline-flex items-center text-[var(--color-electric-blue)] font-semibold group-hover:text-[var(--color-violet-glow)] transition-colors duration-300">Read more <span className="ml-1">→</span></Link>
              </div>
            </article>
          ))}
        </div>
