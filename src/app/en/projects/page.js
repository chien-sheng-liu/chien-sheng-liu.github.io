      ...
      <div className="relative z-10 container mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <motion.div className="text-center mb-16" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
          ...
          <motion.p className="text-xl text-[var(--color-gray-300)] max-w-4xl mx-auto leading-relaxed mb-12" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.4 }}>
            A selection across <span className="text-[var(--color-electric-blue)] font-semibold">Data Science</span>, <span className="text-[var(--color-violet-glow)] font-semibold">Machine Learning</span>, and full‑stack development—showcasing how complex data problems become business outcomes.
          </motion.p>
            ...
            <motion.div key={index} variants={itemVariants} className="relative group overflow-hidden bg-gradient-to-br from-[var(--color-gray-800)]/80 to-[var(--color-gray-800)]/40 backdrop-blur-xl rounded-2xl border border-[var(--color-gray-700)]/50 hover:border-[var(--color-electric-blue)]/50 transition-all duration-500 hover:shadow-2xl hover:shadow-[var(--color-electric-blue)]/10">
              ...
              <div className="relative p-8">
                <div className="flex items-start space-x-4 mb-6">
                  <motion.div className="flex-shrink-0 p-3 bg-[var(--color-electric-blue)]/10 rounded-xl border border-[var(--color-electric-blue)]/20 group-hover:bg-[var(--color-electric-blue)]/20 transition-all duration-300" whileHover={{ rotate: 5, scale: 1.1 }}>
                    {project.icon}
                  </motion.div>
                ...
                <div className="flex flex-wrap gap-2 mb-6">
                  {project.technologies.map((tech, i) => (
                    <span key={i} className="px-3 py-1 bg-[var(--color-electric-blue)]/10 border border-[var(--color-electric-blue)]/20 rounded-full text-xs font-medium text-[var(--color-electric-blue)] backdrop-blur-sm">{tech}</span>
                  ))}
                </div>
        ...
        <motion.div className="text-center" initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }}>
          <div className="max-w-4xl mx-auto">
            <h3 className="text-4xl font-bold text-white mb-6">Want to learn more details?</h3>
            <p className="text-xl text-[var(--color-gray-300)] mb-8 leading-relaxed">Each project has unique challenges and solutions. I’d love to share more technical details, implementation process, and key learnings.</p>
            ...
            <motion.div className="mt-16 pt-8 border-t border-[var(--color-gray-700)]/30" initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ delay: 0.5 }}>
              <div className="flex flex-wrap justify-center items-center gap-8 text-sm text-[var(--color-gray-500)]">
                <div className="flex items-center space-x-2"><FaCode className="text-[var(--color-electric-blue)]" /><span>100% Open Source</span></div>
                <div className="flex items-center space-x-2"><FaStar className="text-[var(--color-violet-glow)]" /><span>Production‑proven</span></div>
                <div className="flex items-center space-x-2"><FaDatabase className="text-[var(--color-electric-blue)]" /><span>Complete docs</span></div>
              </div>
            </motion.div>
          </div>
        </motion.div>
